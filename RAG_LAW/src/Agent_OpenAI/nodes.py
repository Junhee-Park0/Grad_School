...
from src.Agent_OpenAI.functions import load_prompt, determine_input_type, document_ocr, filter_low_relevance_contexts
...
def query_rewriting_node(state: LegalAgentState) -> LegalAgentState:
    """질문을 재작성하는 노드"""
    extended_query_result = extend_query.invoke({"original_query": state["input_query"]})
    print("-" * 50)
    print("✒️  쿼리 재작성")
    print("-" * 50)
    print(f"기존 쿼리 : {state['input_query']}")
    print(f"확장 쿼리 : {extended_query_result}")
    return {"extended_query": extended_query_result}
...
def answer_regenerating_node(state: LegalAgentState) -> LegalAgentState:
    ...
    # should_regenerate와 일치: 최대 3번까지 재생성 (current_retry < 3)
    # current_retry가 3 이상이면 종료
    if current_retry >= 3:
        print("❗답변 재생성 횟수가 최대 횟수(3회)에 도달하였습니다. 가장 높은 신뢰도의 답변을 선택합니다.")
        answer_history = state.get("answer_history", [])
        if answer_history:
            best_answer = max(answer_history, key = lambda x: x.confidence_score)
            print(f"📊 답변 히스토리: {len(answer_history)}개")
            print(f"✅ 선택된 답변의 신뢰도: {best_answer.confidence_score}")
        else:
            best_answer = state.get("answer", None)
            print("⚠️ 답변 히스토리가 없어 현재 답변을 사용합니다.")
        print(f"가장 높은 신뢰도의 답변 : {best_answer.answer[:100]}..." if best_answer and len(best_answer.answer) > 100 else f"가장 높은 신뢰도의 답변 : {best_answer.answer if best_answer else 'None'}")
    ...
    print(f"🔄 재생성 시도 {current_retry + 1}/3")
    ...
    # answer_history 누적 (이전 히스토리에 새 답변 추가)
    previous_history = state.get("answer_history", [])
    updated_history = previous_history + [new_answer]
    
    return {
        "answer": new_answer, 
        "answer_retry_count": current_retry + 1, 
